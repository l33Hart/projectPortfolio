        ...
        public static void ColorStarPrinter(List<RestaurantsReviews> reviews )
        {
            foreach (RestaurantsReviews pairs in reviews)
            {
                if (pairs.Rating <= 2.49m)
                {
                    ...
                    if (pairs.Rating <= 1.49m && pairs.Rating >= 0.50) { Console.ForegroundColor = ConsoleColor.Red; Console.Write("*"); Console.ForegroundColor = ConsoleColor.Gray; Console.WriteLine("****"); }
